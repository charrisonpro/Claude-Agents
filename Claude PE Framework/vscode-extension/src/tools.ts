...
export function executeTool(
    ...
    try {
        switch (toolName) {
            ...
            case 'list_files': {
                const includeArchive = toolInput.include_archive as boolean || false;
                const files = listFilesRecursive(frameworkPath, frameworkPath, includeArchive);
                return `Files:\n${files.join('\n')}`;
            }
            case 'init_project': {
                const projectPath = toolInput.project_path as string;
...
function listFilesRecursive(baseDir: string, currentDir: string, includeArchive: boolean): string[] {
    const files: string[] = [];
...
function initProject(projectPath: string, projectName: string): string {
...
function generateInstructionsTemplate(projectName: string): string {
...
function generateVersionHistoryTemplate(projectName: string): string {
...
function generateConventionsTemplate(projectName: string): string {
...
function generateSMEContentTemplate(projectName: string): string {
